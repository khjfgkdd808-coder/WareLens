...
=====================================================
CLIP 모델을 불러오고, 이미지를 768차원 임베딩 벡터로 변환하는 기능을 담당합니다.

향후 다른 임베딩 모델로 교체하고 싶다면 이 파일만 수정하면 됩니다.
모델을 변경한 경우 반드시 build_vectors.py를 재실행하여 캐시를 갱신해야 합니다.
"""
...
# ViT-L/14: 768차원 임베딩, ViT-B/32(512차원) 대비 정확도 향상
# 모델 변경 후 build_vectors.py 재실행 필수
...
def load_clip_model() -> tuple[CLIPModel, CLIPProcessor]:
    ...
    처음 실행 시 모델을 다운로드하며(약 1.7GB), 이후에는 캐시에서 로드합니다.
...
def get_image_embedding(
    ...
    """
    PIL Image 1장을 768차원 L2 정규화된 임베딩 벡터로 변환합니다.

    처리 순서:
    1. CLIPProcessor로 전처리 (리사이즈, 정규화)
    2. vision_model로 1024차원 특징 추출 (pooler_output)
    3. visual_projection으로 768차원으로 변환
    ...
    Returns:
        np.ndarray: shape (1, 768), L2 정규화된 임베딩 벡터
    """
    ...
    with torch.no_grad():
        ...
        image_features = output.pooler_output                     # (1, 1024)
        image_features = model.visual_projection(image_features)  # (1, 768)

    # L2 정규화: 벡터 크기를 1로 통일 → 코사인 유사도 계산에 최적화
    image_features = F.normalize(image_features, p=2, dim=-1)

    return image_features.cpu().numpy()  # shape: (1, 768)
...
def get_average_embedding(
    ...
    Returns:
        np.ndarray: shape (1, 768), L2 정규화된 평균 임베딩 벡터
    """
    embeddings = []

    for image in images:
        embedding = get_image_embedding(image, model, processor)  # (1, 768)
    ...
    return avg_normalized  # shape: (1, 768)
