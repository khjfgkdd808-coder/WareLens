...
# 캐시 폴더 및 파일 경로 설정
# __file__ 기준 절대경로 사용 → 터미널 실행 위치(cwd)와 무관하게 항상 동일한 폴더를 가리킴
...
def load_cache() -> tuple[np.ndarray, list[str]]:
    ...
    Raises:
        FileNotFoundError: 캐시 파일이 없는 경우.
            CLI(main.py)에서는 이 예외를 잡아 안내 메시지 출력 후 종료하고,
            서버(service.py)에서는 CacheNotFoundError로 변환하여 백엔드에 전달합니다.
    """
    if not cache_exists():
        raise FileNotFoundError(
            "캐시 파일이 없습니다. 먼저 'python build_vectors.py'를 실행해 "
            "데이터셋 임베딩 캐시를 생성하세요."
        )
