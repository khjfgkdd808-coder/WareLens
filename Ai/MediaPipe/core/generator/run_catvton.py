...
# 클래스명 변경 요구사항에 따른 외부 라이브러리와의 이름 충돌 방지 별칭 적용
...
class CatVTONPipeline:
    ...
    def preserve_aspect_ratio_and_pad(self, pil_img: Image.Image, target_size=(768, 1024)) -> Image.Image:
        # 미사용 변수 경고를 없애기 위해 세로 높이 자리에 언더스코어(_) 적용하여 해결
        target_w, _ = target_size
        orig_w, _ = pil_img.size
        
        bg_color = pil_img.getpixel((0, 0))
        
        # 변수 대신 원본 객체의 인덱스를 직접 참조하여 비율 연산 수행
        aspect_orig = orig_w / pil_img.size[1]
        aspect_target = target_w / target_size[1]
        ...
        if aspect_orig > aspect_target:
            ...
            new_h = target_size[1]
            new_w = int(target_size[1] * aspect_orig)
