import type { AnalysisStatus, CategoryFilter, SortKey, AnalysisErrorCode, Season } from '@/types'

export const MAX_CLOTHING_IMAGES  = 5
export const MAX_IMAGE_SIZE_BYTES = 10 * 1024 * 1024

export const ANALYSIS_STEPS: Record<AnalysisStatus, { label: string; description: string; progress: number }> = {
...
export const ANALYSIS_ERROR_MESSAGES: Record<AnalysisErrorCode, { title: string; desc: string; action: string }> = {
  BODY_NOT_DETECTED: { title: '사진에서 인물을 찾지 못했어요.', desc: '전신이 모두 보이는 사진으로 다시 업로드해 주세요.', action: '다시 업로드' },
  LOW_VISIBILITY:    { title: '관절 위치가 잘 보이지 않아요.', desc: '밝은 장소에서 배경이 단순한 환경으로 다시 촬영해 주세요.', action: '다시 촬영' },
  UPLOAD_FAILED:     { title: '업로드에 실패했어요.', desc: '네트워크 상태를 확인하고 다시 시도해 주세요.', action: '다시 시도' },
  NETWORK_ERROR:     { title: '서버와 연결이 끊겼어요.', desc: '잠시 후 다시 시도해 주세요.', action: '다시 시도' },
  UNKNOWN:           { title: '알 수 없는 오류가 발생했어요.', desc: '잠시 후 다시 시도해 주세요.', action: '다시 시도' },
}
...
export const SEASON_FILTERS: Season[] = ['전체', '봄/가을', '여름', '겨울']
